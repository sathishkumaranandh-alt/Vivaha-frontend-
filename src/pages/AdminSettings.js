    ...
    <div style={S.page}>
      ...
      {tab === "site" && (
        <div style={S.card}>
          ...
          <Field label="Homepage Tagline (small text under logo)" value={settings.homepage_tagline} onChange={(v) => handleSettingChange("homepage_tagline", v)} placeholder="Find Your Perfect Life Partner" />
      ...
      {tab === "home" && (
        <div style={S.card}>
          ...
          <div style={S.notice}>
            💡 <strong>Tip:</strong> Changes appear on the homepage after you click Save. The page updates in real-time.
          </div>

          {/* HERO SECTION */}
          <h3 style={S.sectionTitle}>🎬 Hero Section</h3>

          <Field
            label="Eyebrow Text (small text above title)"
          ...
          <Field
            label="Tamil Subtitle (italic serif line)"
            value={settings.home_tamil_subtitle}
            onChange={(v) => handleSettingChange("home_tamil_subtitle", v)}
            placeholder="நம் பாரம்பரியம்... உங்கள் வாழ்க்கைத் துணைக்கு..."
          />
          ...
          <Field
            label="Hero Image URL (right side)"
          ...
          {settings.home_hero_image && (
            <div style={{ marginTop: 10, marginBottom: 20 }}>
              <div style={S.previewLabel}>Image Preview:</div>
              <img
                src={settings.home_hero_image}
                alt=""
                style={{ width: "100%", maxWidth: 300, borderRadius: 10, border: "1px solid #f0e0e0" }}
...
const S = {
  ...
  input: { width: "100%", padding: "11px 14px", borderRadius: 8, border: "1px solid #d1d5db", fontSize: 14, fontFamily: "inherit", boxSizing: "border-box", background: "#FFF9F5", outline: "none" },
  helpText: { fontSize: 12, color: "#888", marginTop: 6, fontStyle: "italic" },
  previewLabel: { fontSize: 11, color: "#888", fontWeight: 600, marginBottom: 6, textTransform: "uppercase" },
