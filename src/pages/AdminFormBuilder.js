  ...
  return (
    <div style={{ maxWidth: "900px", margin: "0 auto", padding: isMobile ? "16px" : "32px" }}>
      ...
      <div style={{ background: "white", borderRadius: "14px", padding: "24px", border: "1px solid #f0e0e0", marginBottom: "24px" }}>
        <h3 style={{ color: "#8B0A2E", marginTop: 0, marginBottom: "16px", fontSize: "16px" }}>➕ Add New Field</h3>
        ...
        {newField.type === "select" && (
          <div style={{ marginBottom: "14px" }}>
            ...
            <input style={S.input} placeholder="Reading, Sports, Music" value={newField.options} onChange={(e) => setNewField({ ...newField, options: e.target.value })} />
          </div>
        )}
              ...
              <div key={f.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: "#FFF9F5", borderRadius: "10px", border: "1px solid #f0e0e0", flexWrap: "wrap", gap: 10 }}>
                <div>
                  <div style={{ fontWeight: 700, color: "#8B0A2E", fontSize: "14px" }}>
                    {f.label} <span style={{ fontWeight: 400, color: "#8a6b6b", fontSize: "12px" }}>({f.field_key})</span>
                  </div>
                ...
                <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                  <button onClick={() => handleToggle(f)} style={{ background: f.is_active ? "#dcfce7" : "#f3f4f6", color: f.is_active ? "#166534" : "#666", border: "none", padding: "6px 12px", borderRadius: "6px", fontSize: "11px", fontWeight: 700, cursor: "pointer" }}>
                    {f.is_active ? "Active" : "Hidden"}
                  </button>
                  <button onClick={() => handleDelete(f.id)} style={{ background: "#fee2e2", color: "#b91c1c", border: "none", padding: "6px 12px", borderRadius: "6px", fontSize: "11px", fontWeight: 700, cursor: "pointer" }}>
                    Delete
                  </button>
                </div>
              </div>
