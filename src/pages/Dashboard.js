  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Load some profiles as "recently viewed" placeholder
        const recRes = await fetch(`${BACKEND_URL}/profile/search`);
        if (recRes.ok) {
          const data = await recRes.json();
          setRecent((data.results || []).filter(u => u.id !== user.id).slice(0, 3));
        }
      } catch (err) {
  ...
  return (
    <div style={S.layout}>
      ...
      <main>
        ...
        <div style={S.rvGrid}>
          {recent.length === 0 ? (
            <p style={{ color: "#8a6b6b", fontSize: "13px" }}>No suggestions yet.</p>
          ) : (
            recent.map((u) => (
