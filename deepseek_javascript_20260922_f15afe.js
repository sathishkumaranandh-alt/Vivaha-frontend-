...
function Profile() {
  ...
  // Interest status
  const [interestStatus, setInterestStatus] = useState("none"); // none | pending | accepted | declined
  const [interestDirection, setInterestDirection] = useState(null); // sent | received
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        // Load interest status (only for other profiles)
        if (!isOwnProfile) {
...
// ============================================================
// INTEREST BUTTON — changes based on status
// ============================================================
function InterestButton({ status, direction, busy, onClick }) {
  ...
  // Case: Received an interest (they sent to me)
  ...
  // Case: Was declined (by them) — allow re-send? No, show declined
  ...
  // Case: I declined them — allow re-send
  if (status === "declined" && direction === "received") {
    return (
      <button
        onClick={onClick}
        disabled={busy}
        style={{ ...interestBtn, background: "linear-gradient(135deg, #dc2626, #f97316)", color: "white", border: "none", cursor: busy ? "not-allowed" : "pointer", opacity: busy ? 0.6 : 1 }}
      >
  ...
  // Default: no interest yet — show Send Interest button
  return (
    <button
      onClick={onClick}
      disabled={busy}
