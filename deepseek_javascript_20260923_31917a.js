...
function ProfileSearch() {
  ...
  const S = {
    ...
    actions: { display: "flex", flexDirection: isMobile ? "row" : "column", gap: "8px", alignItems: isMobile ? "center" : "stretch", minWidth: isMobile ? 0 : "130px", marginTop: isMobile ? "12px" : 0, clear: isMobile ? "both" : "none" },
    sendBtn: {
      background: "#8B0A2E",
      color: "white",
      border: "none",
    ...
    shortlistBtn: (isShortlisted) => ({
      background: isShortlisted ? "#fee2e2" : "white",
      color: isShortlisted ? "#dc2626" : "#8B0A2E",
      border: "1.5px solid " + (isShortlisted ? "#dc2626" : "#8B0A2E"),
      padding: "10px",
      borderRadius: "8px",
      fontSize: "16px",
      cursor: "pointer",
      fontFamily: "inherit",
      flex: isMobile ? "0 0 44px" : "none",
      display: "flex",
      alignItems: "center",
      justifyContent: "center",
    }),
      ...
      <div style={S.layout}>
        ...
        <div>
          ...
          {loading ? (
            ...
            <p style={{ textAlign: "center", color: "#8a6b6b", padding: "60px" }}>Preparing search...</p>
          ) : results.length === 0 ? (
              ...
              {paginated.map((profile) => (
                <div key={profile.id} style={S.card}>
                  ...
                  <div style={S.actions}>
                    ...
                    <Link to={`/profile/${profile.id}`} style={S.viewBtn}>
                      View Profile
                    </Link>
                    <button
                      onClick={() => toggleShortlist(profile.id, profile.name)}
                      style={S.shortlistBtn(shortlistedIds.has(profile.id))}
                      title={shortlistedIds.has(profile.id) ? "Remove from shortlist" : "Add to shortlist"}
                    >
