...
function Profile() {
      ...
      <div style={{ maxWidth: "700px", margin: "0 auto", padding: "20px" }}>
        ...
        <form onSubmit={handleSave} style={{ background: "white", borderRadius: "14px", padding: "24px", border: "1px solid #f0e0e0" }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            <button type="submit" disabled={saving} style={{ flex: 1, background: "#8B0A2E", color: "white", border: "none", padding: "14px", borderRadius: "10px", fontWeight: 700, cursor: "pointer", fontSize: "15px" }}>
              {saving ? "Saving..." : "💾 Save Profile"}
            </button>
            {profileExists && (
              <button type="button" onClick={() => setIsEditing(false)} style={{ background: "#f3f4f6", color: "#374151", border: "none", padding: "14px 24px", borderRadius: "10px", fontWeight: 700, cursor: "pointer", fontSize: "15px" }}>
                Cancel
              </button>
            )}
          </div>
  ...
  const S = {
    ...
    photoMain: {
      background: "linear-gradient(135deg, #ffe0e6, #ffc6d3)",
      borderRadius: "20px",
    ...
    gallery: { display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "6px", marginBottom: "20px" },
    thumb: {
      aspectRatio: "1",
  ...
  return (
    <div style={S.layout}>
      {/* LEFT: PHOTO */}
      <div>
        <div style={S.photoMain}>
          {profile.photo_url ? (
            <img src={profile.photo_url} alt={profile.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          ) : "👤"}
          {profile.is_verified && <div style={S.verifiedBig}>✓ Verified</div>}
          {!isOwnProfile && (
            <button style={S.heartBig} onClick={handleShortlist} title="Shortlist">
              {isShortlisted ? "♥" : "♡"}
            </button>
          )}
        </div>
        <div style={S.gallery}>
          {[1, 2, 3, 4, 5].map((n) => (
            <div key={n} style={{ ...S.thumb, borderColor: n === 1 ? "#D4A017" : "transparent" }}>
              👤
            </div>
          ))}
        </div>
